...
int handle_format_specifier(const char *fmt, va_list args)
{
    ...
    /* @handler_state: phases of format modification */
    enum
		{
			normal_state,
			flag_state,
...
/** string_handler: handle strings
 * Return: returns the length of modified buffer */
...
int string_handler(char *str, int style, int strict_precision, int length_fstring, char *buffer, fmt_flags *flag_switch)
{
    ...
    /** @style_state: string printing style */
    enum style_state
		{
			normal,
			rotate_13,
...
/** get_integer - cast va_arg  to its type according to stage
 * and signess specified
 * Return: @val
 */
...
/** integer_handler - handles integer
